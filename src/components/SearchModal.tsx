    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-24 sm:pt-32">
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-[#141414] rounded-2xl border border-[#1F1F1F] shadow-2xl overflow-hidden">
        <form onSubmit={handleSearch} className="flex items-center gap-4 p-4 border-b border-[#1F1F1F]">
          ...
          />
          {query && (
            <button type="button" onClick={() => setQuery('')} className="p-1 rounded-full hover:bg-[#1F1F1F] text-[#525252]">
              <X className="w-5 h-5" />
            </button>
          )}
          <button type="button" onClick={onClose} className="hidden sm:block px-3 py-1.5 bg-[#1F1F1F] rounded-lg text-xs text-[#A3A3A3]">ESC</button>
        </form>
                ...
                content.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="text-sm font-medium text-[#F5F5F7] line-clamp-2 mb-1">{item.title}</h4>
                      <p className="text-xs text-[#A3A3A3] line-clamp-1">{item.excerpt}</p>
                    </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Technology', 'Climate', 'Sports', 'Markets'].map((term) => (
                    <button key={term} onClick={() => setQuery(term)} className="px-3 py-1.5 bg-[#1F1F1F] rounded-full text-sm text-[#F5F5F7] hover:bg-[#2A2A2A] transition-colors">{term}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="space-y-2">
                  {['AI Breakthrough', 'World Cup', 'Stock Market', 'Climate Summit'].map((term, i) => (
                    <button key={term} onClick={() => setQuery(term)} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-[#1F1F1F] transition-colors text-left">
                      <span className="w-6 h-6 flex items-center justify-center rounded bg-[#D7FF00]/10 text-[#D7FF00] text-xs font-bold">{i + 1}</span>
