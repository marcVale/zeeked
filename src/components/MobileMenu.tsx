    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="absolute right-0 top-0 h-full w-80 max-w-full bg-[#141414] border-l border-[#1F1F1F] shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-[#1F1F1F]">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-[#1F1F1F] text-[#A3A3A3]"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="p-4 border-b border-[#1F1F1F]">
          <span className="text-xs text-[#525252] uppercase tracking-wider mb-2 block">
            Language
          </span>
          <LanguageSelector />
        </div>
              ...
              return (
                <li key={item.label}>
                  <a
                    href={item.href}
                    onClick={onClose}
                    className="flex items-center gap-3 px-3 py-3 rounded-xl text-[#F5F5F7] hover:bg-[#1F1F1F] transition-colors"
                  >
          ...
          <div className="flex flex-wrap gap-2">
            {['Tech', 'Business', 'Sports', 'Entertainment', 'Science', 'Health'].map((cat) => (
              <a
                ...
                onClick={onClose}
                className="px-3 py-1.5 bg-[#1F1F1F] rounded-full text-sm text-[#A3A3A3] hover:text-[#F5F5F7] hover:bg-[#2A2A2A] transition-colors"
              >
                {cat}
              </a>
            ))}
            ...
            {bottomItems.map((item) => {
              ...
              return (
                <li key={item.label}>
                  <a
                    href={item.href}
                    className="flex items-center gap-3 px-3 py-2 rounded-xl text-[#A3A3A3] hover:text-[#F5F5F7] hover:bg-[#1F1F1F] transition-colors"
                  >
