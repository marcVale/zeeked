...
const TrendingSection = ({ t }: TrendingSectionProps) => {
  ...
  return (
    <section ref={sectionRef} className="section-pinned z-20" style={{ background: '#0B0B0D' }}>
      <div ref={imageRef} className="absolute inset-0 overflow-hidden">
        <img src="https://images.unsplash.com/photo-1477959858617-67f85cf4f1df?w=1600&q=80" alt="City skyline" className="w-full h-full object-cover" style={{ filter: 'saturate(0.6) contrast(1.1)' }} />
      ...
      <div className="absolute inset-0 flex flex-col justify-center px-[7vw]">
        <h2 ref={headlineRef} className="heading-display text-[#F5F5F7] mb-6" style={{ fontSize: 'clamp(72px, 10vw, 160px)' }}>{t('section.trending')}</h2>
        <p ref={subheadlineRef} className="text-[#B8B8BD] text-xl max-w-md mb-8" style={{ lineHeight: 1.6 }}>{t('section.trending.subtitle')}</p>
        <div ref={ctaRef} className="flex items-center gap-6">
          ...
          <button className="group flex items-center gap-2 text-[#F5F5F7] font-medium hover:text-[#D7FF00] transition-colors"><Globe className="w-4 h-4" />{t('cta.viewByCountry')}</button>
        </div>
      </div>
